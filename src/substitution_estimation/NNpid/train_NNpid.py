...
from dataloading_utils_NNpid import MyDataset, my_collate

from models_NNpid_utils import AttNet, State, get_params
...
def training_loop(train_loader,val_loader,epochs=101,fname="../models/state.pth",fnameb=None,state=None,last_epoch_sched=float('inf'),use_mut=True,cont_size=CONT_SIZE):
    ...
    for epoch in range(state.epoch, epochs):
        ...
        for X,y,PID,lPID,pos,length,pfreqs,l_pfreqs in train_loader:
            ...
            l = Loss(torch.log(y_hat+1e-40),y)/440 
            l.backward()
            state.optim.step()
...
if __name__ == "__main__":
    ...
    params = get_params(32,6,8,32,4,0.2)
    ...
    fname = "../models/state_NNpid.pth"

    _,_,_ = training_loop(train_dataloader, val_dataloader,fname=fname,epochs=1008,state=state,use_mut=False)