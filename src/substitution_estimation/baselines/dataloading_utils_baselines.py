...
class MyDataset(Dataset):
    def __init__(self, data_dir, cont_size=6,div=2000,verbose=False):
        ...
        self.profiles = [] #frequencies of each symbol in each column of a family (of size [l,20])
                ...
                with open(temp_path, newline='') as f:
                    ...
                    while line:
                        ...
                        if line[0] == '>': #header line
                            ...
                            prot_aa_freq = torch.ones(1,20)
                            n += 1
                            if newf and not first_prot:
                                newf = False
                            first_prot = False
                            col_idx = 0 #new protein -> reset the column idx
                            ...
                            for aa in line:
                                ...
                                if newf:
                                    profile.append(torch.ones(1,20))
                                    
                                if aa_id < 20:
                                    aa_freq[aa_id] += 1
                                    prot_aa_freq[0][aa_id] += 1
                                    profile[col_idx][0][aa_id] += 1
    ...
    def __getitem__(self, idx, sample_size='auto',rep=rep,rand=rand): 
        ...
        for _ in range(sample_size):
                ...
                for aa_i, aa_j in zip(line_i,line_j):
                    ...
                    if aa_i == aa_j:
                        if aa_i != '-':
                            ...
                            seq_i += aa_i
                            seq_j += aa_j 
                            profile.append(self.profiles[idx][col_idx])
                    else:
                        seq_i += aa_i
                        seq_j += aa_j
                        profile.append(self.profiles[idx][col_idx])
            ...
            for _ in range(rep_number):
                ...
                pos.append(pos_ij)
                couples.append(torch.tensor([i,j]))
                ...
                for w in range(k-self.cont_size,k+self.cont_size+1):
                    if w < 0 or w >= align_l: #case of the edges
                        window_i += 'Z'
                        window_j += 'Z'
                        window_profile.append(torch.ones(1,20))
                    else:
                        window_i += seq_i[w]
                        window_j += seq_j[w]
                        window_profile.append(profile[w])
                ...
                window_profile[self.cont_size][0][y_j] -= 1 #discardind the target from the count

                profiles.append(torch.cat(window_profile))
        ...
        couples = torch.stack(couples)
        ids = repeat(torch.tensor([idx]),'1 -> b',b = len(lengths))

        out = X,y.long(),PIDs,local_PIDs,pos,lengths,pfreqs,local_pfreqs,profiles,ids, couples
        return out
...
def my_collate(batch):
    ...
    profiles = torch.cat([item[8] for item in batch],dim=0)
    ids = torch.cat([item[9] for item in batch],dim=0)
    couples = torch.cat([item[10] for item in batch],dim=0)

    return data, target, PID, lPID, pos, length, pfreqs, l_pfreqs, profiles,ids, couples