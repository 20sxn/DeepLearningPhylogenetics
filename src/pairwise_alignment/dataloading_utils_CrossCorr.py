    ...
    def __getitem__(self, idx, sample_size='auto',rep=rep,rand=rand): 
        ...
        for _ in range(sample_size):
            ...
            for offset in range(sequence_line_count-1): #computing PID and removing aligned '-' ##might need to compute the actual column num
                ...
                full_seq1 += line_i.replace('-', '')
                full_seq2 += line_j.replace('-', '')
            ...
            if len(full_seq1) < len(full_seq2): full_seq1,full_seq2 = full_seq2,full_seq1
                
            full_seq1 = [ALPHABET.get(i, 21) for i in full_seq1]
            full_seq2 = [ALPHABET.get(i, 21) for i in full_seq2]
            
            full_seq1 = torch.tensor(full_seq1)
            full_seq1 = F.one_hot(full_seq1,22)
            full_seq1[:,21] = 0
            full_seq1 = full_seq1.float()
            
            full_seq2 = torch.tensor(full_seq2)
            full_seq2 = F.one_hot(full_seq2,22)
            full_seq2[:,21] = 0
            full_seq2 = full_seq2.float()
            ...
            for _ in range(rep_number):
                ...
                full_seqs_1.append(full_seq1)
                full_seqs_2.append(full_seq2)
        ...
        out = X,y.long(),PIDs,full_seqs_1,full_seqs_2,pos,lengths,pfreqs,local_pfreqs
        return out
...
def my_collate(batch):
    ...
    full_seqs1 = [item for sublist in batch for item in sublist[3]]
    full_seqs2 = [item for sublist in batch for item in sublist[4]]
    pos = torch.cat([item[5] for item in batch],dim=0)
    length = torch.cat([item[6] for item in batch],dim=0)
    pfreqs = torch.cat([item[7] for item in batch],dim=0)
    l_pfreqs = torch.cat([item[8] for item in batch],dim=0)
    return data, target, PID, full_seqs1,full_seqs2, pos, length, pfreqs, l_pfreqs