...
class AttNet(nn.Module):
    ...
    def cross_corr_slow(self,seq1,seq2):
        ...
        for idx in range(N):
            l1 = seq1[idx].shape[0]
            l2 = seq2[idx].shape[0]
            output = F.conv1d(self.crosscorr_norm(self.embedding(seq1[idx].to(self.F.device))).T.unsqueeze(0),
                              self.crosscorr_norm(self.embedding(seq2[idx].to(self.F.device))).T.unsqueeze(1), 
                              stride=1,padding=l2-1,groups=self.embedding_dim).squeeze(0)
            output = output[:,output.sum(dim=0).argmax()]/l2
    ...
    def cross_corr(self,seq1,seq2):
        N = len(seq1)
        sseq1 = [self.crosscorr_norm(self.embedding(seq1[idx].to(self.F.device))) for idx in range(N)]
        sseq2 = [self.crosscorr_norm(self.embedding(seq2[idx].to(self.F.device))) for idx in range(N)]

        ss1 = nn.utils.rnn.pad_sequence(sseq1,batch_first=True).to(self.F.device)
        ss2 = nn.utils.rnn.pad_sequence(sseq2,batch_first=True).to(self.F.device)

        N,l1,emb_dim = ss1.shape

        ll2 = torch.tensor([len(seq2[i]) for i in range(N)],device=self.F.device)    

        weights = ss2.transpose(1,2).reshape(N*emb_dim,1,-1)

        # move batch dim into channels
        x = ss1.transpose(1,2).reshape(1,N*emb_dim, -1)

        output = F.conv1d(x, weights, padding=ss2.shape[1]-1, groups=N*emb_dim)

        out = output.view(N,emb_dim,-1)
        #print(out)
        idx = out.sum(dim=1).argmax(dim=1)
    ...
    def to_input(self,x,pfreqs,l_pfreqs):
        ...
        lpf = lpf[:,0]
        out = torch.cat([out,pf,lpf],dim=1)
