...
from dataloading_utils_CrossCorr import MyDataset, my_collate
...
if __name__ == "__main__":
    ...
    fname = '../substitution_estimation/data/test_dataset.pth'
    savepath = Path(fname)
    if not savepath.is_file():
        test_dataset = MyDataset(r"../substitution_estimation/data/test_data",cont_size = CONT_SIZE,div=2000)
        with savepath.open("wb") as fp:
    ...
    fname = '../substitution_estimation/data/train_dataset.pth'
    savepath = Path(fname)
    if not savepath.is_file():
        train_dataset = MyDataset(r"../substitution_estimation/data/train_data",cont_size = CONT_SIZE,div=2000)
        with savepath.open("wb") as fp:
            torch.save(test_dataset,fp)
    else:
        with savepath.open("rb") as fp:
    ...
    savepath = Path(fname)
    if not savepath.is_file():
        val_dataset = MyDataset(r"../substitution_estimation/data/val_data",cont_size = CONT_SIZE,div=2000)
        with savepath.open("wb") as fp:
            torch.save(test_dataset,fp)
    else:
        with savepath.open("rb") as fp:
    ...
    model = AttNet(*params,clf_dims=[],embedding_dim=128)
